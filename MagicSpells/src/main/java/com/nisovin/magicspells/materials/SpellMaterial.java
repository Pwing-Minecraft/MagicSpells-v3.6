...
/**
 * Created by Redned on 4/29/2019.
 */
public enum SpellMaterial {
    ...
    SpellMaterial(int data, String... materials) {
    ...
    public boolean isDamageable(SpellMaterial type){
        if (type == null)
            return false;
    ...
    private static Map<String, SpellMaterial> cachedSearch = new HashMap<String, SpellMaterial>();

    public static SpellMaterial fromMaterial(Material mat){
        try {
            return SpellMaterial.valueOf(mat.toString());
        } catch (IllegalArgumentException ex) {
            for (SpellMaterial type : SpellMaterial.values()){
                for (String test : type.materials) {
    ...
    public static SpellMaterial fromString(String key){
        String matKey = key.replace("minecraft:", "").toUpperCase();
        SpellMaterial mat = null;

        try {
            mat = SpellMaterial.valueOf(matKey);
            return mat;
        } catch (IllegalArgumentException ex) {
            String[] split = matKey.split(":");
            if (split.length == 1)
                mat = requestMaterial(matKey,  0);
            else
    ...
    public static SpellMaterial requestMaterial(String name, int data) {
        if (cachedSearch.containsKey(name.toUpperCase() + "," + data))
            return cachedSearch.get(name.toUpperCase() + "," + data);

        for (SpellMaterial mat : SpellMaterial.values()) {
        ...
        if (id != null) {
            Material mat = MagicSpells.getVolatileCodeHandler().fromId(id, (byte) data);
            if (mat != null) {
                SpellMaterial spellMat = SpellMaterial.fromMaterial(mat);
                cachedSearch.put(id + "," + data, spellMat);
                return spellMat;
            }
        }
    ...
    private static Material getMaterialFromID(int id, byte data) {
        return MagicSpells.getVolatileCodeHandler().fromId(id, data);
    }
