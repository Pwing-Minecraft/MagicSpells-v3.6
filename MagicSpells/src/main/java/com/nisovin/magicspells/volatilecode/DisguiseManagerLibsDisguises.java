...
public class DisguiseManagerLibsDisguises implements Listener, IDisguiseManager {
	...
	private me.libraryaddict.disguise.disguisetypes.Disguise getDisguiseLibDisguise(DisguiseSpell.Disguise dis) {
		...
		if (disType.isMob()) {
			...
			if (w instanceof CreeperWatcher) {
				CreeperWatcher c = (CreeperWatcher)w;
				c.setPowered(dis.getFlag());
			} else if (w instanceof ZombieVillagerWatcher) {
				if (dis.getVar1() >= 1) {
					((ZombieVillagerWatcher)w).setProfession(dis.getVar1());
				}
				...
				if (dis.getVar1() == -1) {
					((SheepWatcher)w).setColor(AnimalColor.getColorByDye(this.random.nextInt(16)));
				} else if (dis.getVar1() >= 0 && dis.getVar1() < 16) {
					((SheepWatcher)w).setColor(AnimalColor.getColorByDye(dis.getVar1()));
				}
				...
				if (dis.getVar1() > 0) {
					...
					wolfWatch.setCollarColor(AnimalColor.getColorByDye(dis.getVar1()));
				}
