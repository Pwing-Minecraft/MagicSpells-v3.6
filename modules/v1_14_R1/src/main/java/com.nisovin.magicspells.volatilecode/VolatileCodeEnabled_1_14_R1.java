...
public class VolatileCodeEnabled_1_14_R1 implements VolatileCodeHandle {
	...
	public VolatileCodeEnabled_1_14_R1(MagicConfig config) {
		try {
	...
	private NBTTagCompound getTag(ItemStack item) {
		...
		if (item instanceof CraftItemStack) {
			try {
				return ((net.minecraft.server.v1_14_R1.ItemStack)this.craftItemStackHandleField.get(item)).getTag();
			} catch (Exception e) {
				// No op currently
			}
		}
	...
	private ItemStack setTag(ItemStack item, NBTTagCompound tag) {
		...
		net.minecraft.server.v1_14_R1.ItemStack nmsItem = null;
		try {
			nmsItem = (net.minecraft.server.v1_14_R1.ItemStack)this.craftItemStackHandleField.get(item);
		} catch (Exception e) {
			// No op currently
		}
	...
	public boolean createExplosionByPlayer(Player player, Location location, float size, boolean fire, boolean breakBlocks) {
		Explosion.Effect effect = breakBlocks ? Explosion.Effect.BREAK : Explosion.Effect.NONE;
		return !((CraftWorld)location.getWorld()).getHandle().createExplosion(((CraftPlayer)player).getHandle(), location.getX(), location.getY(), location.getZ(), size, fire, effect).wasCanceled;
	}
	...
	public Fireball shootSmallFireball(Player player) {
		net.minecraft.server.v1_14_R1.World w = ((CraftWorld)player.getWorld()).getHandle();
	...
	public void createFireworksExplosion(Location location, boolean flicker, boolean trail, int type, int[] colors, int[] fadeColors, int flightDuration) {
		// Create item
		net.minecraft.server.v1_14_R1.ItemStack item = CraftItemStack.asNMSCopy(new ItemStack(SpellMaterial.FIREWORK_ROCKET.parseMaterial()));
	...
	public void playDragonDeathEffect(Location location) {
		EntityEnderDragon dragon = new EntityEnderDragon(EntityTypes.ENDER_DRAGON, ((CraftWorld)location.getWorld()).getHandle());
	...
	public void setClientVelocity(Player player, Vector velocity) {
		PacketPlayOutEntityVelocity packet = new PacketPlayOutEntityVelocity(player.getEntityId(), new Vec3D(velocity.getX(), velocity.getY(), velocity.getZ()));
		((CraftPlayer)player).getHandle().playerConnection.sendPacket(packet);
	}
